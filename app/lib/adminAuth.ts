 ...
 * Admin sessions.
 *
 * Identity comes from Google; this module only decides who is allowed in and
 * issues the session. The session is an HMAC-signed, expiring cookie carrying
 * the editor's email, so saves are attributed to a person rather than to a
 * shared login. There is no user table: who may sign in is whoever appears in
 * ADMIN_EMAILS.
 */
